...
def _stable_db_path(source_path: Path, cache_dir: Path | None) -> Path:
    ...
    digest = hashlib.sha256(
        f"{source_path.resolve()}::{source_path.stat().st_mtime_ns}::{source_path.stat().st_size}".encode("utf-8")
    ).hexdigest()[:16]
...
def _extract_gene_records(db: gffutils.FeatureDB) -> dict[str, dict[str, object]]:
    ...
    for feature in db.all_features(order_by=("seqid", "start", "end")):
        ...
        raw_gene_id = _first_attr(feature, ["gene_id", "ID", "Name", "gene", "gene_name"]) or feature.id
...
def _build_gene_model_from_db(db: gffutils.FeatureDB) -> GeneModel:
    ...
    all_transcripts = sorted(set(exon_groups.keys()) | set(cds_groups.keys()) | set(transcript_meta.keys()))
    for transcript_id in all_transcripts:
        ...
        if cds_records:
            ...
            for cds_feature in cds_records:
                ...
                if ftype == "cds":
                    cds = CDS(cds_feature.start, cds_feature.end, chrom, strand, _feature_attr_map(cds_feature))
                elif ftype == "five_prime_utr":
                    cds = FP_UTR(cds_feature.start, cds_feature.end, chrom, strand, _feature_attr_map(cds_feature))
                else:
                    cds = TP_UTR(cds_feature.start, cds_feature.end, chrom, strand, _feature_attr_map(cds_feature))
...
def write_intron_cache(model: GeneModel, outdir: str | Path) -> Path:
    ...
    with cache_path.open("w", encoding="utf-8") as handle:
        for gene in sorted(model.getAllGenes(), key=lambda g: (g.chromosome, g.minpos, g.maxpos, g.id)):
