...
def _segment(name: str, start: int, cigar: tuple[tuple[int, int], ...], query_len: int) -> pysam.AlignedSegment:
...
def build_alignment_fixture(tmp_path: Path, *, repeat_scale: int = 120) -> AlignmentFixture:
    ...
    segments = [_segment(f"r{idx}", start, cigar, query_len) for idx, (start, cigar, query_len) in enumerate(reads)]
    ...
    with pysam.AlignmentFile(cram, "wc", header=header, reference_filename=str(reference_fasta)) as out:
        for seg in segments:
            out.write(seg)
